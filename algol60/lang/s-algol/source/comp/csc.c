...
read_string()     /* read a string terminated by " into the_string */
{
	...
	while ( peep != '"' && !feof( si ) )
	{
		t->class.str.str_pntr = mk_string( read_char(),nil ) ;
...
psint	checked ;
{
	...
	{
		head->class.link.left = enter( ( pntr ) head->class.link.left,temp,checked ) ;
		return( head ) ;
	} else
	if ( strcmp( temp->class.link.name,head->class.link.name ) > 0 )
	{
		head->class.link.right = enter( ( pntr ) head->class.link.right,temp,checked ) ;
		return( head ) ;
	} else
...
psint	checked ;
{
	...
	env_list->class.cons.hd = enter( ( pntr ) env_list->class.cons.hd,mk_link( n,lex_level,add2,t,nil,nil ),checked ) ;
}
