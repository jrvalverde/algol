...
pntr	struct_param()
{
	...
	do {
		t1 = type() ;
		do {
			t->class.namelist.next = mk_namelist( the_name,t1,nil ) ;
			check_name( the_name ) ;
			mustbe( identifier_sy ) ;
...
pntr	struct_decl()
{
	...
	if ( have( lp_sy ) )
	{
		...
		while ( t->skind != NIL )
		{
			ptypes->class.cons.tl = mk_cons( ( pntr )t->class.namelist.types,nil ) ;
			...
			{
				list->class.namelist.next = mk_namelist( t->class.namelist.names,( pntr )t->class.namelist.types,nil ) ;
...
structure_decl()
{
	...
	a = struct_decl() ;
	struct_list->class.struct_table.str_next = mk_struct_table( a->class.struct_table.s_tdm,a->class.struct_table.s_ptrs,nil ) ;
...
psint	function ;
{
	...
	if ( !have( rp_sy ) )
	{
		...
		if ( !res )
		{
			...
			while ( have( comma_sy ) )
			{
				if ( function ) t->class.cons.tl = mk_cons( param_type( FALSE ),nil ); else t->class.cons.tl = mk_cons( type(),nil ) ;
...
pntr	param_decl()
{
	...
	while( more1 )
	{
		more2 = TRUE ;
		t1 = param_type( TRUE ) ;
		t->class.cons.tl = mk_cons( t1,nil ) ;
		t = ( pntr ) t->class.cons.tl ;
...
pntr	proc_decl()
{
	...
	the_type->class.proc.args = arg_types ; the_type->class.proc.result = restype ;
