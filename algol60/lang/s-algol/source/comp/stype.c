...
pntr	sty ;
{
	...
	p->class.struct_table.str_next = sty ;
	return( p ) ;
}
...
pntr	nt ;
{
	...
	p->class.EXTERNAL.e_type = t ;
	p->class.EXTERNAL.e_addr = a ;
	p->class.EXTERNAL.e_next = nt ;
	return( p ) ;
}
...
pntr	t ;
{
	...
	p = ( pntr ) malloc( stype_size ) ;
	p->skind = IMAGE_CLASS ;
	p->class.Image.image_elements = t ;
	return( p ) ;
}
#endif	RASTER
...
pntr	t,l,r ;
{
	...
	p->class.link.type1 = t ;
	p->class.link.left = l ;
	p->class.link.right = r ;
	return( p ) ;
}
...
pntr	t,n ;
{
	...
	p->class.namelist.types = t ;
	p->class.namelist.next = n ;
	return( p ) ;
}
